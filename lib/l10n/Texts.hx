...
/**
 * Текстовое хранилище.  
 * Класс содержит все локализованные тексты приложения
 * в одном месте.
 * 1. Словари `Dictionary` с исходными текстами предварительно
 *    необходимо добавить вызовом метода: `add()`.
 * 2. Вы можете добавлять новые словари или изменять тексты
 *    уже добавленных в любое время.
 */
class Texts
{
    /**
     ...
     *   хранится в: `window.texts`
     * - Хранилище инициализируется при первом доступе к нему.
     * - Через это свойство вы можете получить доступ к ресурсам,
     *   загруженных **другим** Haxe/JS приложением.
     * 
     * Не может быть `null`
     */
     ...
     * Парсит переданные данные в формате CSV и возвращает
     * локализацию.
     * 
     * Описание формата:
     * - Строки, начинающиеся с символа # - игнорируются.
     *   (Это комментарии)
     * - Одна строка - одна текстовая фраза. (Если вам нужен
     *   перенос в тексте, используйте: `\n`)
     * - Первая запятая в строке интерпретируется как разделитель
     *   ключа и текстовой фразы, последующие запятые игнорируются.
     * - Ключ текстовой фразы должен быть положительным, целым
     *   числом: `unsigned Int32`
     ...
     * @param sep Разделитель фразы и её текста. (Можно изменить при необходимости)
    ...
    /**
     * Получить текст.  
     * Возвращает локализованный текст или указанное значение `none`,
     * если текста с таким ID не найдено. (Может быть `null`)
     ...
     * Влияет на то, какой текст будет возвращаться в ответ
     * на вызов методов: `at()` и `Texts.get()`
     * 
     * По умолчанию: `en`
     */
    ...
    /**
     * Добавить слова.  
     * Заносит в словарь указанной локализации новые текстовые
     * данные, обновляет старые.
     ...
     * Получить текст.  
     * Возвращает локализованный текст или указанное значение `none`,
     * если текста с таким ID не найдено.  
     * (Может быть `null`)
     * 
